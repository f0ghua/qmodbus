...
/* Reads the data from a remove device and put that data into an array */
static int read_registers32(modbus_t *ctx, int function, int addr, int nb,
                          uint32_t *dest)
{
    int rc;
    int req_length;
...
/* Reads the holding registers of remote device and put the data into an
   array */
int modbus_read_registers32(modbus_t *ctx, int addr, int nb, uint32_t *dest)
{
    int status;
    ...
    status = read_registers32(ctx, MODBUS_FC_READ_HOLDING_REGISTERS,
                            addr, nb, dest);
    return status;
}
